...
CPropertyEditor::CPropertyEditor(QWidget* parent)
    : QTreeWidget(parent), m_addingItem(false)
{
}
...
bool CPropertyEditor::remove(CBaseProperty* prop)
{
    ...
    if (!m_propertyMap.contains(prop->getId()))
    {
        qDebug() << "Property with id=" << prop->getId()
...
void CPropertyEditor::onCurrentItemChanged(QTreeWidgetItem* current,
...
void CPropertyEditor::onItemChanged(QTreeWidgetItem* item, int column)
{
    ...
    if (prop != NULL)
    {
        if (column)
        {
            qDebug() << "Value state of property [" << prop->getId()
                     << "] changed to: " << prop->getVariantValue();
        }
        else
        {
            if (prop->isMarkable())
                qDebug() << "Marked state of property [" << prop->getId()
                         << "] changed to: " << prop->isMarked();
        }
    }
}
